...
//         pool.query('SELECT * FROM users WHERE id=?',[companyId] ,(err, comapanyInfoResult) => {
//             if (err) {
//                 console.error('Error executing SQL query for category:', err);
//                 return res.status(500).json({ error: 'Internal Server Error' });
//             }

//             if (comapanyInfoResult.length === 0) {
//                 return res.status(404).json({ status: 404, error: 'No comapany name found' });
//             }

//                 res.status(200).json({ status: 200, data: comapanyInfoResult });
//         });
//     } catch (error) {
